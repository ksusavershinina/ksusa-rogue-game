...
class ItemAction(Action):
    ...
    def perform(self) -> None:
        """Вызвать способность предметов, это действие будет
         предоставлено для предоставления контекста."""
...
class ActionWithDirection(Action):
    ...
    def dest_xy(self) -> Tuple[int, int]:
        """Возращает место назначения данного действия"""
        return self.entity.x + self.dx, self.entity.y + self.dy

    @property
    def blocking_entity(self) -> Optional[Entity]:
        """Возвращает блокирующий объект в этом месте назначения действий."""
        return self.engine.game_map.get_blocking_entity_at_location(*self.dest_xy)

    @property
    def target_actor(self) -> Optional[Actor]:
        """Возвращает исполнителя действия в новые координаты после
        исполнения действия"""
...
class MovementAction(ActionWithDirection):
    ...
    def perform(self) -> None:
        ...
        # в границах ли действие
        if not self.engine.game_map.in_bounds(dest_x, dest_y):
            raise exceptions.Impossible("That way is blocked.")
        # можно ли идти вообще
        if not self.engine.game_map.tiles["walkable"][dest_x, dest_y]:
            raise exceptions.Impossible("That way is blocked.")
        # нет ли сущностей, которые блокируют путь
